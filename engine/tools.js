...
export const tools = {
  getLineTiles: (start, end) => {
    ...
    let tCol = end.col, tRow = end.row;
    if (dCol > dRow) tRow = start.row;
    else tCol = start.col;

    const dist = Math.max(Math.abs(tCol - start.col), Math.abs(tRow - start.row));
  ...
  // Outline rect (keep for reference, no longer used by default)
  getRectTiles: (start, end) => {
    const tiles = [];
    const rStart = Math.min(start.row, end.row), rEnd = Math.max(start.row, end.row);
    const cStart = Math.min(start.col, end.col), cEnd = Math.max(start.col, end.col);
  ...
  // Generic painter — used for immediate tile and drag-release shapes
