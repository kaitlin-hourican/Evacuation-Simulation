...
export class StatsController {
    ...
    INJURY_SCALE.forEach(item => {
      this.injuryBars[item.level] = document.querySelector(`.injury-bar[data-level="${item.level}"]`);
      
      const row = document.createElement('div');
      row.className = 'legend-item';
      
      // Left side: Wrapper for dot+label | Right side: Number
      row.innerHTML = `
      ...
      this.injuryTexts[item.level] = document.getElementById(`ais-${item.level}`);
    });
}
  ...
  #formatTime(seconds) {
    ...
    return `${Math.floor(sec/60).toString().padStart(2,'0')}:${(sec%60).toString().padStart(2,'0')}.${ms}`;
  }
}
