...
export class FireSystem {
  ...
  // Get visible tile range from scroll position (same as Grid.draw)
  const wrapper = this.grid.wrapper;
  const tileSize = this.grid.tileSize;

  const startCol = Math.floor(wrapper.scrollLeft / tileSize);
  const endCol   = Math.ceil((wrapper.scrollLeft + wrapper.clientWidth) / tileSize);
  const startRow = Math.floor(wrapper.scrollTop / tileSize);
  const endRow   = Math.ceil((wrapper.scrollTop + wrapper.clientHeight) / tileSize);
  ...
  ctx.restore();
    ...
    #initIntensityField() {
        this.intensityField = Array.from({ length: this.grid.rows }, () =>
            Array.from({ length: this.grid.cols }, () => ({ temp: 0, state: FIRE_STATES.COOL }))
        );
    }
    ...
    #seedFire() {
        ...
        for (const { row, col } of this.grid.getFireCells()) {
            this.intensityField[row][col] = { temp: this.flashpoint, state: FIRE_STATES.BURNING };
            queue.push({ row, col });
        }
    ...
    spread() {
        ...
        const nextField = Array.from({ length: this.grid.rows }, () => new Array(this.grid.cols));

        // loop through intensityField
        for (let row = 0; row < this.grid.rows; row++) {
            for (let col = 0; col < this.grid.cols; col++) {
                ...
                for (const [dr, dc] of neighbours) {
                  ...
                  // 4. Update State and Clamp Values
            let nextState = FIRE_STATES.COOL;
            
            if (nextTemp >= this.flashpoint) {
                nextState = FIRE_STATES.BURNING;
            } else if (nextTemp > 20) {
                nextState = FIRE_STATES.HEATING; // Assumes you have a warming state
            }

            // Cap temp at 255 for your RGB draw logic
            nextField[row][col] = { 
